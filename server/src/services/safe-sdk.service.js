...
export async function createSafeTransaction({ to, value = '0', data = '0x' }, signerPk) {
...
export async function executeTransaction(safeTransactionData, signatures, executorPk) {
  ...
  for (const sig of signatures) {
    ...
    safeTransaction.addSignature(sigStr);
  }

  const isValid = await protocolKit.isValidTransaction(safeTransaction);
  if (!isValid) {
    throw new Error('Transaction validation failed - check signatures and Safe state');
  }
